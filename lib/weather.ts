...
export async function getFarmWeather():Promise<WeatherResult>{
  ...
  try{
    ...
    const response=await fetch(url,{next:{revalidate:1800},signal:AbortSignal.timeout(8000)});if(!response.ok)throw new Error("provider error");const raw=await response.json() as {current?:Record<string,number>;daily?:Record<string,Array<string|number>>};const current=raw.current??{},dailyRaw=raw.daily??{};
