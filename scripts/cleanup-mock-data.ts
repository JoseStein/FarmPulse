...
async function main() {
  ...
  const [tasks, activities, expenses, notes, issues, budgets, inventory, equipment, recommendations, alerts, notifications, weather] = await Promise.all([
    ...
    prisma.equipment.findMany({ where: { farmId }, select: { id: true, name: true, notes: true } }),
  ...
  const mock = {
    ...
    equipment: equipment.filter((row) => MOCK_EQUIPMENT.includes(row.name) && !(row.notes ?? "").includes("May 2024")),
