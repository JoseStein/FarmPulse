...
import {createActivityAction,createExpenseAction,createFieldNoteAction,adjustStockAction,updateIssueStatusAction,updateTaskStatusAction,createFarmUserAction,updateFarmUserAccessAction,changeOwnPasswordAction,removeInactiveFarmUserAction} from "@/app/actions";
...
describe.sequential("database-backed pilot workflows",()=>{
 ...
 it("lets a user change only their own password",async()=>{const userId=created.users[0];authState.userId=userId;const newPassword="Changed-integration-2026!";const result=await changeOwnPasswordAction({currentPassword:"Integration-user-2026!",newPassword,confirmPassword:newPassword});expect(result.ok).toBe(true);const user=await prisma.user.findUniqueOrThrow({where:{id:userId}});expect(await bcrypt.compare(newPassword,user.passwordHash)).toBe(true);expect(await prisma.auditLog.findFirst({where:{userId,action:"PASSWORD_CHANGE",entityId:userId}})).not.toBeNull()});
 ...
 it("removes only the inactive farm membership and preserves the historical user",async()=>{authState.userId=adminId;const userId=created.users[0];const result=await removeInactiveFarmUserAction({userId});expect(result.ok).toBe(true);expect(await prisma.farmMembership.findFirst({where:{userId}})).toBeNull();expect(await prisma.user.findUnique({where:{id:userId}})).toMatchObject({id:userId,active:false});expect(await prisma.auditLog.findFirst({where:{action:"USER_REMOVE",entityId:userId}})).not.toBeNull()});
