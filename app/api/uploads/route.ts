...
export async function POST(request:Request){
  try{
    ...
    const stored:Array<Awaited<ReturnType<typeof storeImage>>>=[];let attachments;
    try{
      for(const file of files)stored.push(await storeImage(file,context.farm.id));
      attachments=await prisma.$transaction(async tx=>{const rows=[];for(const item of stored)rows.push(await tx.attachment.create({data:{...item,fieldNoteId:note.id}}));await tx.auditLog.create({data:{farmId:context.farm.id,userId:context.user.id,action:"UPLOAD",entityType:"FieldNote",entityId:note.id,metadata:{count:rows.length}}});return rows;});
    }catch(error){await Promise.allSettled(stored.map(item=>deleteStoredImage(item.storageKey)));throw error}
